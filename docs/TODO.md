...
## Design
- [ ] Optional values: nullable types (`int?`, `string?`). `Nullable<T>` needs primitive support, and `string?` cannot be told apart from `string` at runtime.
- [ ] Record in DESIGN.md: the primitive's type decides whether null is allowed. A null default on a non-nullable primitive fails, and there's no special handling of null in the pipeline.
